...
def infer_data_type(values: list[str]) -> str:
    ...
    for value in values:
        if re.match(r"^-?\d+$", value):
            ...
            values_types["STRING"] += 1
            
    sorted_types = sorted(values_types.items(), key=lambda k: k[1], reverse=True)
    return sorted_types[0][0] if sorted_types else "STRING"

def capitalize(string: str) -> str:
    """Capitalizes the first character without changing the rest"""
        ...
        if k and k.lower() != "id": ## Attribute name is not "None" nor "id"
            if table.columns.get(k, False) or k in [k[0] for k in table.foreign_keys]:
                ...
                if score < threshold:
                    ...
                    table.add_column(k, v)
                    table.add_foreign_key(k, table_name, f"_id")
...
def generate_relational_model(objects: list[Entity_Object], threshold = int):
    ...
    for obj in objects: ## First pass - create entities
        entity_table = tables[capitalize(obj.name)]
        entity_table.name = capitalize(obj.name)

        for aggregate_name in obj.aggregates.keys(): ## 1:1
            aggregate_table = tables[capitalize(aggregate_name)]
            aggregate_table.name = capitalize(aggregate_name)
        
        for agg_arr_name in obj.aggregate_arrays.keys(): ## 1:N, N:N
            agg_arr_table = tables[capitalize(agg_arr_name)]
            agg_arr_table = capitalize(agg_arr_name)
    ...
    for obj in objects: ## Second pass - set PKs, FKs, and processes attributes
        entity_name = capitalize(obj.name)

        entity_table = tables[entity_name]
        entity_table.count += 1
        entity_table.add_column(f"_id", "SERIAL")  ## PK = _id (U-schema format)
        entity_table.set_primary_key(f"_id")
        ...
        for aggregate_name, attributes in obj.aggregates.items(): ## 1:1
            aggregate_name = capitalize(aggregate_name)

            aggregate_table = tables[aggregate_name]
            aggregate_table.count += 1
            aggregate_table.add_column(f"_id", "SERIAL") ## PK = _id (U-schema format)
            aggregate_table.set_primary_key(f"_id")

            # FK goes to the main entity table, as "Entity HAS aggregate"
            entity_table.add_column(f"{aggregate_name}ID", 999)  ## FK = aggregate.name+ID
            entity_table.add_foreign_key(f"{aggregate_name}ID", aggregate_name, f"_id") ## FK on table, REF table, column on REF table
        ...
        for agg_arr_name, agg_arr in obj.aggregate_arrays.items(): ## For each aggregate array in object. Ex: watchedMovies{], favoriteMovies{}
            ...
            agg_arr_name = capitalize(agg_arr_name)
            ...
            for attributes in agg_arr.values(): ## For each instance in aggregate array. Ex: watchedMovie[0]
                ...
                agg_arr_table.add_column(f"{entity_name}ID", 999)
                agg_arr_table.add_foreign_key(f"{entity_name}ID", entity_name, f"_id") ## Adds main entity as FK
                ...
                if score >= threshold:
                    ...
                    if fk_attrib[0] > threshold: ## N:N with entity_ID attribute
                        ...
                        agg_arr_table.add_foreign_key(fk_attrib[1], table_name, f"_id") ## Add identified fk_attrib as FK
                        agg_arr_table.set_primary_key((f"{entity_name}ID", fk_attrib[1])) ## Composite PK (entity_ID, fk_attrib_ID)
                        ...
                        agg_arr_table.add_column(f"{table_name}ID", 999)
                        agg_arr_table.add_foreign_key(f"{table_name}ID", table_name, f"_id") # FK on table, REF table, column on REF table
                        agg_arr_table.set_primary_key((f"{entity_name}ID", f"{table_name}ID")) ## Composite PK (entity_ID, table_name_ID)

                else: ## Handle 1:N relationship
                    agg_arr_table.add_column("_id", "SERIAL")
                    agg_arr_table.set_primary_key("_id") ## PK = _id
...
def generate_sql_schema(tables: defaultdict[str, Table]):
    ...
    for table_name, table in tables.items():
        ...
        if not composite_pk:
            columns.append(f"{table.primary_key} INT PRIMARY KEY")
